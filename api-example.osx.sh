...
generateJwtToken()
{
    ...
    jwt_payload=$(echo -n \{\"iat\":`date +%s`,\"exp\":`date -v +30S +%s`,\"sub\":\"api_key_jwt\",\"iss\":\"external\",\"jti\":\"`cat /dev/urandom | LC_CTYPE=C tr -dc 'A-Z0-9'| fold -w 12 | head -n 1`\"\}| base64 | tr -d '=' | tr '/+' '_-' | tr -d '\n')
